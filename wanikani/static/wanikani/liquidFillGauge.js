...
function loadLiquidFillGauge(elementId, value, config) {
    if(config == null) { console.log("Using ...."); config = liquidFillGaugeDefaultSettings(); }
    ...
    var gaugeCircleY = d3.scaleLinear().range([0, radius]).domain([0, 1]);
    ...
    var waveRiseScale = d3.scaleLinear()
        ...
        // such that it will overlap the fill circle at all when at 0%, and will totally cover the
        // fill-circle at 100%.
    ...
    // Scale for controlling the position of the text within the groups
     ...
     var text1 = gaugeGroup.append("text")
        .text(textRounder(textStartValue) + percentageText)
     ...
     if(config.valueCountUp){
        var textTween = function() {
            ...
            return function(t) {that.text(textRounder(i(t)) + percentageText).append('tspan').attr('x', 0).attr('dy', '-1.2em').text(config.level); }
        };
